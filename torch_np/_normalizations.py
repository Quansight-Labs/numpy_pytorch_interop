...
def postprocess_ndarray(result):
    return _helpers.array_from(result)


def postprocess_out(result):
    result, out = result
    kwds = postprocess_out.kwds
    return _helpers.result_or_out(result, out, **kwds)


def postprocess_tuple(result):
    return _helpers.tuple_arrays_from(result)


def postprocess_list(result):
    return list(_helpers.tuple_arrays_from(result))


def postprocess_variadic(result):
...
def normalizer(_func=None, *, return_on_failure=_sentinel, promote_scalar_out=False):
    def normalizer_inner(func):
        ...
        def wrapped(*args, **kwds):
            ...
            if postprocess:
                postprocess.kwds = {"promote_scalar": promote_scalar_out}
